...
from train import train, car_train, wocar_train
...
parser.add_argument(
    ...
    default=True,
    metavar='CAR',
    help='whether to train a CAR-DQN model')
parser.add_argument(
    '--solver',
    default='cov',
    metavar='SOLVER',
    help='method to solve the inner maximization, one of {pgd, cov}')
parser.add_argument(
    '--soft-lambda',
    type=float,
    default=1.0,
    metavar='SL',
    help='the soft coefficient for CAR-DQN model')
parser.add_argument(
    '--trades',
    default=False,
    metavar='TRADES',
    help='whether to use the trade loss when training a CAR-DQN model')
parser.add_argument(
    '--episodic',
    default=False,
    metavar='EPISODIC',
    help='whether to use the episode life wrapper when training a CAR-DQN model')
parser.add_argument(
    '--radial',
    default=True,
    metavar='RADIAL',
    help='whether to train a RADIAL-DQN model')
parser.add_argument(
    '--worst-rate',
    type=int,
    default=10,
    help='worst model training frequency when training a WocaR-DQN model')
parser.add_argument(
    '--worst-ratio',
    type=float,
    default=0.05,
    help='ratio of worst-case q value when training a WocaR-DQN model')
parser.add_argument(
    '--worse-bound',
    default=True,
    help='if this is selected worst case loss uses bound that is further away from mean when training a WocaR-DQN model')
...
if __name__ == '__main__':
    ...
    if "NoFrameskip" not in args.env:
        ...
        if args.episodic:
            env = wrap_deepmind(env, central_crop=True, clip_rewards=False, episode_life=True, **env_conf)
        else:
    ...
    if args.car==True:
        print("Training a CAR-DQN model")
        car_train(curr_model, target_model, env, args)
    elif args.radial==True:
        print("Training a RADIAL-DQN model")
        train(curr_model, target_model, env, args)
    else:
        print("Training a WocaR-DQN model")
        worst_model = DuelingCnnDQN(env.observation_space.shape[0], env.action_space)
        if args.gpu_id >= 0:
            with torch.cuda.device(args.gpu_id):
                curr_model.cuda()
                target_model.cuda()
                worst_model.cuda()
        wocar_train(curr_model, target_model, worst_model, env, args)
