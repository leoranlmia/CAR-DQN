...
def eval_greedy_wc(curr_model, env, args, epsilon=1e-4):
    ...
    with torch.no_grad():
        for _ in range(args.max_episode_length):
            ...
            # Find the actions that are impossible to be optimal under any attack
...
def eval_q_error(curr_model, env, args, save_path, save_name, seed, epsilon=0.0):
    episode_reward = 0
    state = env.reset()
    q_value_seq = []
    reward_seq = []
    ...
    np.save('{}/evaluate/{}/q_error_{}.npy'.format(save_path, save_name, seed), errors)
    
    plt.legend()
    plt.xlabel('Step')
    plt.savefig('{}/evaluate/{}/q_error_{}.png'.format(save_path, save_name, seed))
    plt.close()
...
def plot_results(epsilons, rewards, eval_type, save_path, save_name):
    ...
    plt.ylabel('reward')
    #plt.xscale('log')
    np.save('{}/evaluate/{}/{}.npy'.format(save_path, save_name, eval_type), rewards)
    plt.savefig('{}/evaluate/{}/{}.png'.format(save_path, save_name, eval_type))
    plt.close()
...
if __name__ == '__main__':
    ...
    save_name = '{}_{}episodes_{}epiLength'.format(save_name, args.num_episodes, args.max_episode_length)
    save_path = args.load_path.rsplit('/', 1)[0]
    ...
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    if not os.path.exists(save_path+'/evaluate'):
        os.mkdir(save_path+'/evaluate')
    if not os.path.exists(save_path+'/evaluate/'+save_name):
        os.mkdir(save_path+'/evaluate/'+save_name)

    log = {}
    setup_logger('{}_log'.format(args.env), r'{}/evaluate/{}/log'.format(
        save_path, save_name))
    ...
    if args.video:
        ...
        anim.save(save_path+'/evaluate/'+save_name+'_nominal.gif', writer='imagemagick', fps=20)
    ...
    if args.pgd_video:
        ...
        anim = animation.FuncAnimation(plt.gcf(), animate, frames=len(frames), interval=50)
        anim.save(save_path+'/evaluate/'+save_name+'_pgd_{}.gif'.format(args.pgd_video), writer='imagemagick', fps=20)
    ...
    if args.nominal:
        ...
        np.save('{}/evaluate/{}/nominal.npy'.format(save_path, save_name), rewards)
        plt.savefig('{}/evaluate/{}/nominal.png'.format(save_path, save_name))
        plt.close()

        log['{}_log'.format(args.env)].info('Nominal mean reward +- sem:{} +- {}'.format(np.mean(rewards), np.std(rewards)/np.sqrt(np.shape(rewards))))

    epsilons = [1/255, 3/255, 5/255]
    if args.fgsm:
        log['{}_log'.format(args.env)].info('Evaluating against FGSM with epsilons:{}'.format(epsilons))
        np.save('{}/evaluate/{}/fgsm_epsilons.npy'.format(save_path, save_name), epsilons)
        rewards = []
        ...
        plot_results(epsilons, rewards, 'fgsm', save_path, save_name)
        log['{}_log'.format(args.env)].info('FGSM mean rewards: {}'.format(np.mean(rewards, axis=1)))
        log['{}_log'.format(args.env)].info('FGSM rewards(sem): {}'.format(np.std(rewards, axis=1) / np.sqrt(np.shape(rewards)[1])))
        
    if args.pgd:
        log['{}_log'.format(args.env)].info('Evaluating against PGD with epsilons:{}'.format(epsilons))
        np.save('{}/evaluate/{}/pgd_epsilons.npy'.format(save_path, save_name), epsilons)
        rewards = []
        for epsilon in epsilons:
            print(epsilon)
            curr_rewards = []
        ...
        plot_results(epsilons, rewards, 'pgd', save_path, save_name)
        log['{}_log'.format(args.env)].info('PGD mean rewards: {}'.format(np.mean(rewards, axis=1)))
        log['{}_log'.format(args.env)].info('PGD rewards(sem): {}'.format(np.std(rewards, axis=1)/np.sqrt(np.shape(rewards)[1])))
        
    if args.gwc:
        log['{}_log'.format(args.env)].info('Evaluating using GWC with epsilons:{}'.format(epsilons))
        np.save('{}/evaluate/{}/greedy_wc_epsilons.npy'.format(save_path, save_name), epsilons)
        rewards = []
        for epsilon in epsilons:
            print(epsilon)
            curr_rewards = []
        ...
        plot_results(epsilons, rewards, 'greedy_wc', save_path, save_name)
        log['{}_log'.format(args.env)].info('GWC mean rewards: {}'.format(np.mean(rewards, axis=1)))
        log['{}_log'.format(args.env)].info('GWC rewards(sem): {}'.format(np.std(rewards, axis=1) / np.sqrt(np.shape(rewards)[1])))
        
    if args.acr:
        log['{}_log'.format(args.env)].info('Evaluating using ACR with epsilons:{}'.format(epsilons))
        np.save('{}/evaluate/{}/acr_epsilons.npy'.format(save_path, save_name), epsilons)
        rates = []
        for epsilon in epsilons:
            print(epsilon)
            curr_rates = []
        ...
        plot_results(epsilons, rates, 'acr', save_path, save_name)
        log['{}_log'.format(args.env)].info('ACR mean rates: {}'.format(np.mean(rates, axis=1)))
        log['{}_log'.format(args.env)].info('ACR rates(sem): {}'.format(np.std(rates, axis=1) / np.sqrt(np.shape(rates)[1])))
    ...
    if args.q_error:
        ...
        for i in range(args.num_episodes):
            set_seed(i)
            mean_error, abs_error = eval_q_error(model, env, args, save_path, save_name, i, epsilon=0)
