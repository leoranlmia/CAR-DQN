...
def plot(frame_idx, rewards, losses, standard_losses, worst_case_losses, args, start_time):
    ...
    ax2.plot(_moving_avg(losses, window=1000), label='loss')
    ax2.plot(_moving_avg(standard_losses, window=1000), label='standard')
    ax2.plot(_moving_avg(worst_case_losses, window=1000), label='worst_case')
    
    ax2.set_yscale('log')
    ax2.legend(loc = 'lower left')
    if not os.path.exists(args.log_dir):
        os.mkdir(args.log_dir)
    plt.savefig('{}/{}_training_{}.png'.format(args.log_dir, args.env, start_time))
    plt.close()
