...
from train import get_logits_lower_bound, logits_margin
...
def negative_q_logits_max(logits, y):
    q_logits_max = torch.max(logits)

    return - q_logits_max

def negative_q_logits_softmax(logits, y):
    tem_lab = 0.1
    p = torch.exp(logits / tem_lab).detach()
    p /= p.sum()
    q_logits_max = p * logits

    return - q_logits_max.sum()
...
def main(args):
    ...
    logger.log(args)
    logger.log(config)
    # configurate the environment parameter
    ...
    logger.log('resetting env with seed', seed)
    env.seed(seed)
    state = env.reset()
    ...
    for frame_idx in range(1, num_episodes * max_frames_per_episode + 1):
        ...
        if test_config['attack']:
            ...
            if test_config['test_loss'] == 'Margin':
                state_tensor = attack(model, state_tensor, attack_config, logits_margin)
            elif test_config['test_loss'] == 'Qmax':
                state_tensor = attack(model, state_tensor, attack_config, negative_q_logits_max)
            elif test_config['test_loss'] == 'SoftQmax':
                state_tensor = attack(model, state_tensor, attack_config, negative_q_logits_softmax)
            else:
