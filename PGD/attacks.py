...
def pgd(model, X, y, verbose=False, params={}, env_id=""):
    ...
    y = Variable(torch.tensor(y))   # ??? Why to make y Variable?
    if verbose:
        print('epislon: {}, step size: {}, target label: {}'.format(epsilon, step_size, y))
    rand = params.get('random_start', True)
    # Initialize X_adv
    # If random_start, initialize X_adv as X adding a noise from uniform [-epsilon, epsilon].
    # If not, initialize X_adv as a copy of X.
    if rand:
    ...
    for i in range(niters):
        # Compute logits by passing X_adv through the model.
        logits = model.forward(X_adv)
        # Compute loss by applying the loss_func to logits and y.
        loss = loss_func(logits, y)
        if verbose:
            print('current loss: ', loss.data.cpu().numpy())
        # Compute the gradient of loss with respect to X_adv and store it in X_adv.grad.
        model.features.zero_grad()
        loss.backward()
        # Update X_adv with gradient ascent step
        # infinity norm attack
        eta = step_size * X_adv.grad.data.sign()
        X_adv = Variable(X_adv.data + eta, requires_grad=True)
        # Project X_adv onto the feasible set:
        # 1. between img_min and img_max (box constraints)
        # 2. not deviate from X by more than epsilon (L-infinity constraints)
...
# get the adversarial state of state X
def attack(model, X, attack_config, loss_func=nn.CrossEntropyLoss()):
    ...
    y = model.act(X)    # select the optimal action
    if method == 'cw':
        atk = cw
    elif method == 'fgsm':
        atk = fgsm
...
# make the label as the input of attack()
def attack_label_as_input(model, X, y, attack_config, loss_func=nn.CrossEntropyLoss()):
    method = attack_config.get('method', 'pgd')
    verbose = attack_config.get('verbose', False)
    params = attack_config.get('params', {})
    params['loss_func'] = loss_func
...
def attack_td_loss(model, X, y, action, attack_config, loss_func=nn.SmoothL1Loss()):
    method = attack_config.get('method', 'pgd')
    verbose = attack_config.get('verbose', False)
    params = attack_config.get('params', {})
    params['loss_func'] = loss_func

    adv_X = pgd_attack_td_loss(model, X, y, action, verbose=verbose, params=params)
    abs_diff = abs(adv_X.cpu().numpy()-X.cpu().numpy())
    if verbose:
        print('adv image range: {}-{}, ori action: {}, adv action: {}, l1 norm: {}, l2 norm: {}, linf norm: {}'.format(torch.min(adv_X).cpu().numpy(), torch.max(adv_X).cpu().numpy(), model.act(X)[0], model.act(adv_X)[0], np.sum(abs_diff), np.linalg.norm(abs_diff), np.max(abs_diff)))
    return adv_X

def pgd_attack_td_loss(model, X, y, action, verbose=False, params={}, env_id=""):
    epsilon = params.get('epsilon', 0.00392)
    if env_id == "CartPole-v0":
        epsilon = torch.from_numpy(CARTPOLE_STD) * epsilon
    if env_id == "Acrobot-v1":
    ...
    for i in range(niters):
        # Compute logits by passing X_adv through the model.
        logits = model.forward(X_adv)
        cur_q_value = logits.gather(1, action.unsqueeze(1)).squeeze(1)
        # Compute loss by applying the loss_func to logits and y.
        loss = loss_func(cur_q_value, y)
        if verbose:
            print('current loss: ', loss.data.cpu().numpy())
        # Compute the gradient of loss with respect to X_adv and store it in X_adv.grad.
        model.features.zero_grad()
