...
import cpprb
import re
from attacks import attack, attack_label_as_input, attack_td_loss
import random
...
class TimeLogger(object):
    ...
    def get_value(self, time_id):
        return 0.0 if time_id not in self.time_logs else self.time_logs[time_id]
...
def logits_margin(logits, y):
    '''
    returns a scalar that represents the sum of the margin for each state-action pair in the batch
    '''
    # mask out the optimal actions
    comp_logits = logits - torch.zeros_like(logits).scatter(1, torch.unsqueeze(y, 1), 1e10)
    # get the second best actions
    sec_logits, _ = torch.max(comp_logits, dim=1)
    margin = sec_logits - torch.gather(logits, 1, torch.unsqueeze(y, 1)).squeeze(1)
    margin = margin.sum()
    return margin

def distance_divide_eps(a, b, epsilon):
    '''
    compute the average lp (1, 2, inf) norm of (a - b) for a batch, and divide by epsilon
    '''
    dist = a - b
    l1 = torch.linalg.norm(dist, ord=1, dim=1).mean()
    l2 = torch.linalg.norm(dist, ord=2, dim=1).mean()
    linf = torch.linalg.norm(dist, ord=float('inf'), dim=1).mean()
    return l1/epsilon, l2/epsilon, linf/epsilon

def linf_dist_divide_eps(a, b, epsilon):
    """compute the average l_inf distance of (a - b)/epsilon for a batch."""
    dist = a - b
    linf = torch.linalg.norm(dist, ord=float('inf'), dim=1).mean()
    return linf/epsilon

def divide_res(a, b):
    if np.abs(b) < np.finfo(np.float32).tiny:
        return 1e11
    else:
        return a / b
...
def compute_td_loss_inf(logger, current_model, target_model, batch_size, replay_buffer, per, use_cpp_buffer, use_async_rb, optimizer, gamma, memory_mgr, robust, **kwargs):

    # Step 1: Sample a batch of transitions from replay buffer
    t = time.time()
    dtype = kwargs['dtype']
    if per:
        buffer_beta = kwargs['buffer_beta']
    ...
    if robust:
        eps = kwargs['eps']
    cur_q_value = cur_q_logits.gather(1, action.type(torch.int64).unsqueeze(1)).squeeze(1)

    # compute the TD-target
    double = kwargs.get('double', False)
    if double:
        if robust and bound_solver != 'pgd':
            cur_next_q_logits = current_model(next_state, method_opt="forward")
        else:
            cur_next_q_logits = current_model(next_state)
        next_best_action = cur_next_q_logits.argmax(1)
        tgt_next_q_value = tgt_next_q_logits.gather(1, next_best_action.unsqueeze(1)).squeeze(1)
    else:
        tgt_next_q_value = tgt_next_q_logits.max(1)[0]
    expected_q_value = reward + gamma * tgt_next_q_value * (1 - done)

    # compute the TD-loss (huber or MSE)
    if kwargs['natural_loss_fn'] == 'huber':
        loss_fn = torch.nn.SmoothL1Loss(reduction='none')
        loss = loss_fn(cur_q_value, expected_q_value.detach())
    else:
    ...
    # Step 3: Compute the robust regularizer
    if robust:
        # when epsilon in robust training is equal to 0, robust loss = 0 (no robust training)
        if eps < np.finfo(np.float32).tiny:
            reg_loss = torch.zeros(state.size(0))
            if USE_CUDA:
                reg_loss = reg_loss.cuda()
            ...
            kwargs['attack_config']['params']['epsilon'] = eps

            # hinge margin
            adv_state = attack_td_loss(current_model, state, expected_q_value.detach(), action, kwargs['attack_config'], nn.SmoothL1Loss())
            optimizer.zero_grad()
            adv_q_logits = current_model.forward(adv_state)
            adv_q_value = adv_q_logits.gather(1, action.unsqueeze(1)).squeeze(1)
            reg_loss = nn.SmoothL1Loss(reduction='none')(adv_q_value, expected_q_value.detach())
            if per:
                reg_loss = reg_loss * weights

            soft_lambda = kwargs.get('soft_lambda', 1.0)
            p = torch.exp(reg_loss / soft_lambda).detach()
            p /= p.sum()
            reg_loss = reg_loss * p
            ...
            # summation of a batch of adv margin, -M_{hat{s}}
            adv_margin = logits_margin(current_model.forward(adv_state), labels)
            # summation of a batch of ori margin, -M_s
            ori_margin = logits_margin(current_model.forward(state), labels)
    ...
    if per:
        res = (loss, grad_norm, weights_norm, td_loss.mean(), batch_cur_q_value, batch_exp_q_value)
    else:
        res = (loss, grad_norm, td_loss.mean(), batch_cur_q_value, batch_exp_q_value)
    if robust:
        if bound_solver == 'pgd':
            res += (ori_margin, adv_margin)
        res += (reg_loss.mean(),)
...
def test_return(model, config, logger, dtype, seed, robust=False, num_episodes=20, max_frames_per_episode=10000):
    logger.log('start robust test')
    training_config = config['training_config']
    env_params = training_config['env_params']
    env_params['clip_rewards'] = False
    env_params['episode_life'] = False
...
def main(args):
    ...
    if robust:
        ...
        reg_losses = []
        # a matrix that stores the indices of all possible actions except the current one.
        # use it to select the actions that are not equal to the current action.
    ...
    for frame_idx in range(load_frame, num_frames + 1):
        ...
        eps = 0
        # get the eps of adversarial or robust training
        ...
        if adv_train and eps != np.nan and eps >= np.finfo(np.float32).tiny:
            ...
            # adv_ratio??? where is it in paper?
            if random.random() < adv_ratio:
                attack_count += 1   # counts of attack
                # state = state after perturbation
                # cross entropy???
                ...
                if current_model.act(state_tensor)[0] != current_model.act(ori_state_tensor)[0]:
                    suc_count += 1  # counts of successful attack
            else:
        ...
        # Step 3: save to buffer (for synchronous env)
        ...
        # Step 4: train
        # start to train when numbers of samples in buffer > replay_initial
        if buffer_stored_size > replay_initial:
            # if robust:
            #     if eps > np.finfo(np.float32).tiny:
            #         logger.log('\nframe_idx: {}'.format(frame_idx))
            ...
            # res: results
            sadqn = training_config.get('sadqn', False)
            if sadqn:
                ...
                res = compute_td_loss_inf(logger, current_model, target_model, batch_size, replay_buffer, per, use_cpp_buffer,
                                  use_async_rb, optimizer, gamma, memory_mgr, robust, buffer_beta=buffer_beta,
                                  grad_clip=grad_clip, natural_loss_fn=natural_loss_fn, eps=eps, beta=beta, sa=sa,
                                  kappa=kappa, dtype=dtype, hinge=hinge, hinge_c=training_config.get('hinge_c', 1),
                                  env_id=env_id, bound_solver=bound_solver, attack_config=attack_config, 
                                  soft_lambda=training_config.get('soft_lambda', 1.0), double=training_config.get('double', False),
                                  trades=training_config.get('trades', False))

            if per:
                loss, grad_norm, weights_norm, td_loss, batch_cur_q_value, batch_exp_q_value = res[0], res[1], res[2], res[3], res[4], res[5]
            else:
                loss, grad_norm, td_loss, batch_cur_q_value, batch_exp_q_value = res[0], res[1], res[2], res[3], res[4]
        ...
        # Step 5: Update states and episode reward
        t = time.time()
        state = next_state
        episode_reward += reward
        if done:
        ...
        # # test natural and robust return
        # if frame_idx % 100000 == 0:
        #     test_natural_reward = test_return(current_model, config, logger, dtype, seed)
        #     all_natural_return.append(test_natural_reward)
        #     np.save('{}/natural_returns.npy'.format(prefix), all_natural_return)

        #     test_robust_reward = test_return(current_model, config, logger, dtype, seed, robust=True)
        #     all_robust_return.append(test_robust_reward)
        #     np.save('{}/robust_returns.npy'.format(prefix), all_robust_return)

        # Step 6: All kinds of result logging
        if frame_idx % training_config['print_frame'] == 0 or frame_idx==num_frames or (robust and abs(frame_idx-schedule_start) < 5) or abs(buffer_stored_size-replay_initial) < 5:
            ...
            logger.log('total: {:.4f}, act_time: {:.4f}, env_time: {:.4f}, sample_time: {:.4f}, nn_time: {:.4f}, reweight_time: {:.4f}, loss_time: {:.4f}, save_time: {:.4f}'.format(
                log_time.get_value('total'), log_time.get_value('act_time'), log_time.get_value('env_time'), log_time.get_value('sample_time'), log_time.get_value('nn_time'), 
                log_time.get_value('reweight_time'), log_time.get_value('loss_time'), log_time.get_value('save_time')))
